...
export class OrganizationClient extends EventEmitter {
  ...
  async joinChannel() {
    try {
      ...
      const completedPromise = joinedChannelPromises.concat([this._channel.joinChannel(request)]);
  ...
  async checkInstalled(chaincodeId, chaincodeVersion, chaincodePath) {
    ...
    return chaincodes.some(
      cc => cc.name === chaincodeId && cc.path === chaincodePath && cc.version === chaincodeVersion
    );
  }
